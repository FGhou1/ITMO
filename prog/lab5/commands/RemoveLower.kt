...
class RemoveLower(private val console: StandardConsole,
    ...
    private fun removeGreaterThan(id: Int) {
        ...
        if (moviesToRemove.isEmpty()) {
            console.printer("Нет фильмов с id меньше $id.")
            return
        }

        moviesToRemove.forEach { movieCollection.removeMovie(console, it) }
        console.printer("Удалено ${moviesToRemove.size} фильмов с id меньше $id.")
    }
}
